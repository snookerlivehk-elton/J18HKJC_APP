...
with st.expander("⚙️ 參數調節", expanded=False):
    ...
    ModelConfig.SYNERGY_SMOOTH_C = st.number_input(
        "騎練平滑常數", min_value=1, value=ModelConfig.SYNERGY_SMOOTH_C, help=ph.smooth_c("騎練合作")
    )
    decay_str = st.text_input(
        "騎練時間衰減",
        value=",".join(map(str, ModelConfig.SYNERGY_DECAY)),
        help=ph.decay("騎練合作"),
    )
