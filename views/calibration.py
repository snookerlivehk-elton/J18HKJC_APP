...
st.markdown(
    f"""
**正確流程**
1. **賽前**：建立預測快照（鎖住當日各因子／總分／模型勝率／**融合廣告推介列**）  
2. **賽後**：J18 歷史爬蟲入庫名次後，執行「結算快照」（回填名次＋獨贏賠率；並寫命中率日快照）  
3. 本頁用已結算快照統計各訊號 **WIN／PLA／WQ／PQ／T3／T4**，供調 `WEIGHT_*`  
4. **廣告宣傳素材**請到「命中率榜」③：四項高賠／覆蓋原則  
...
推介隻數與賽日速覽相同（場內份額 + `select_picks_by_share`）。總榜請優先看「命中率榜」日快照。
"""
)
