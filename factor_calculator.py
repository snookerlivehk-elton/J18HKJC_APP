...
class FactorCalculator:
    ...
    def fetch_historical_data(self) -> pd.DataFrame:
        ...
        query = text(f"""
            SELECT 
                m.racing_date,
                r.course, r.track, r.distance_m, r.ground, r.class as race_class,
        ...
        # 建立基礎分桶 (Bucket): 例如 ST_草地_A_1200
        # 由於 API 欄位有時包含引號 (如 '"C" 賽道') 或空值，需清理並轉字串
        df['course_clean'] = df['course'].fillna('未知').astype(str)
        df['track_clean'] = df['track'].fillna('').astype(str).str.replace('"', '').str.replace(' 賽道', '').str.strip()
        df['bucket_id'] = df['course_clean'] + "_" + df['track_clean'] + "_" + df['distance_m'].astype(str)
